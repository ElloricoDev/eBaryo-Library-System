...
<template>
  ...
  <div class="container py-4">
    ...
    <div class="card mb-4 border-success shadow admin-profile-card">
      <div class="card-body">
        ...
        <p><strong class="text-success"><i class="bi bi-person-badge"></i> Role:</strong> {{ user.role || 'N/A' }}</p>
        <div class="mb-3">
          <label class="form-label fw-bold text-success"><i class="bi bi-mortarboard"></i> Student:</label>
          <span>{{ user.student === 'yes' ? 'Yes' : 'No' }}</span>
        </div>

        <Link :href="route('admin.profile.edit')" class="btn btn-success mt-3 w-100 shadow-sm">
...
.admin-profile-card:focus-within, .admin-profile-card:hover {
